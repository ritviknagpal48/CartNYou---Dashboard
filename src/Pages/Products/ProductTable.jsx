      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
        <div
          className={clsx(classes.tags, {
            "bg-green-100 text-green-900": admin === "approved",
            "bg-red-100 text-red-900": admin === "rejected",
            "bg-yellow-100 text-yellow-900": admin === "pending",
          })}
        >
          {admin}
        </div>
...
const ProductTable = ({ heading, rows }) => {
  return (
    <div className={classes.wrapper}>
      <div className="flex flex-col w-full pr-14 pl-4  my-2">
              ...
              <table className="min-w-full divide-y divide-gray-200 bg-white">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    {heading &&
                      heading.map((head) => (
                        <th scope="col" className={clsx(classes.table_heading)}>
                          {head}
                        </th>
                      ))}
                    <th scope="col" className={clsx(classes.table_heading)}>
                ...
                <tfoot className={"bg-white w-full py-4 px-8"}>
                  <tr className={"text-gray-600 bg-white text-right"}>
                    <td
                      className={
                        "py-4 px-8 w-full flex flex-row items-center justify-start"
                      }
                      colSpan={heading ? heading.length + 1 : 0}
                    >
                      <button
                        type={"button"}
                      ...
                      >
                        <svg
                          className={classes.pagination_icons}
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                      ...
                      <div
                        className={
                          "mx-1 px-4 ring-1 text-center text-sm ring-offset-gray-400 rounded-sm"
                        }
                      >
                        1
                      </div>
